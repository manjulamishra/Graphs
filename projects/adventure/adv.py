...
def has_unexplored_exits(n,g):
    if n in g:
        for exit_direction,room_id in g[n].items():
            if room_id == "?":
                return True, exit_direction
    return False, '?'            

def direction_to_child(parent_dict, child):
    for direction, node in parent_dict.items():
        if node == child:
            return direction

def opposite_direction(given_direction):
    opposite_directions = {'e':'w', 'w':'e', 'n':'s', 's':'n'}
    return opposite_directions[given_direction]


def shortest_path_to_unxlopred_exit(n,g):
    if n not in g:
        return []
    parents ={n:-1}
    to_visit = deque([n])
    unexplored_node_found = False
    while len(to_visit) > 0:
        curr_node = to_visit.popleft()
        if has_unexplored_exits(curr_node, g)[0]:
            unexplored_node_found = True
            break
        for neigbor in g[curr_node].values():
            if neigbor in parents:
                continue
            to_visit.append(neigbor)
            parents[neigbor] = curr_node

    # 
    path = []
    if unexplored_node_found:
        while parents[curr_node] != -1:
            parent_node = parents[curr_node]
            direction = direction_to_child(g[parent_node], curr_node)
...
g = {0:{'e':'?','w':'?','n':'?','s':'?'}}
while len(g) < 500:
    # explore until we reach a node with no unexplored exits
    curr_room_id = player.currentRoom.id
    print(player.currentRoom)
    explore_further, direction_to_travel = has_unexplored_exits(curr_room_id, g)
    if explore_further:
        traversalPath.append(direction_to_travel)
        player.travel(direction_to_travel)
        g[curr_room_id][direction_to_travel] = player.currentRoom.id
        if player.currentRoom.id not in g:
            g[player.currentRoom.id] = {e:'?' for e in player.currentRoom.getExits()}
        g[player.currentRoom.id][opposite_direction(direction_to_travel)] = curr_room_id
    else:
        for reverse_direction in shortest_path_to_unxlopred_exit(curr_room_id,g):
            traversalPath.append(reverse_direction)
            player.travel(reverse_direction)
            print(player.currentRoom)
    
# g = {0:{'n':1, 'e':'?','w':'?','s':'?'}, 1:{'s':0,'e':'?','w':'15','n':2}, 2:{'s':1}, 15:{'e':1,'w':16}, 16:{'e':15, 'n':17}, 17:{'s':16}}

# print(has_unexplored_exits(1,g)[0])
# print(has_unexplored_exits(2,g)[0])
# print(has_unexplored_exits(0,g)[0])
# print()
